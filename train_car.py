    ...
    for epoch in range(epochs):
        ...
        for step, data in enumerate(train_data):
            ...
            if (step+1) % print_step == 0:
                print("Step %d/%d  Loss: %.4f" % (step+1, num_batch, epoch_loss/(step+1)))

        print("Epoch %d/%d Finished!   Train loss: %.4f"%(epoch+1, epochs, epoch_loss / (step+1)))
...
if __name__ == "__main__":
    ...
    dataset_train = torch.utils.data.Subset(carvana_dataset, indices[:num_train])
    dataset_val = torch.utils.data.Subset(carvana_dataset, indices[num_train:])
